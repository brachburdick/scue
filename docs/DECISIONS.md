...
## ADR-021: Live Strata tier — Pioneer-only arrangement analysis
Date: 2026-03-24
Context: The existing Strata tiers (quick, standard, deep) all require an audio file for offline analysis. During a live DJ set, we want arrangement visualization immediately when a track loads on hardware, before any audio is available. Pioneer CDJs already export rich structural data: phrase analysis (section boundaries + labels), 3-band RGB waveform, beat grid, and cue points.
Decision: Add a "live" tier that constructs `ArrangementFormula` entirely from Pioneer hardware data via `LiveStrataAnalyzer.build_from_pioneer(PlayerState)`. Phrases map to sections, phrase boundaries generate transitions, the Pioneer RGB waveform becomes a single "mix" stem, and cue points become markers. The formula is built in `PlaybackTracker` on track load and broadcast via `strata_live` WebSocket message + `GET /api/strata/live` REST endpoint. The frontend polls the REST endpoint every 2s (to handle the race where WS broadcast fires before clients connect) and also receives WS pushes for instant updates. The Live tier button in StrataPage reads from `strataLiveStore` (Zustand) merged with REST polling data. `ArrangementMap` accepts a `playbackCursorTime` prop for a green vertical cursor line with past-section dimming.
Consequences: The live tier has no patterns (no audio = no pattern discovery) and a single stem. Its `pipeline_tier="live"` and `analysis_source="pioneer_live"` distinguish it from offline tiers. The `fingerprint` is synthetic (`"live_{rekordbox_id}"`) — not a real audio hash. Live strata is transient (in-memory only, not persisted to disk). Storage validation allows "live" tier and "pioneer_live" source but they're only used for in-flight formulas, not file storage.
...
## ADR-022: Bidirectional bridge command channel for track scanning
Date: 2026-03-25
Context: SCUE's bridge (ADR-005) is read-only — it streams Pro DJ Link data from Pioneer hardware to Python but cannot send commands back. When a track is physically loaded on a CDJ, beat-link Finders produce rich data (phrase analysis, verified beatgrid, cue points, waveform) not available from USB scanning. Manually loading every track is impractical for a full library. beat-link provides `VirtualCdj.sendLoadTrackCommand()` and `MenuLoader` APIs that BLT (Beat-Link-Trigger) uses for remote track loading.
Decision: Extend the bridge WebSocket with a bidirectional command channel. Commands flow Python → Java as JSON with `{ "command", "request_id", "params" }` envelopes; responses flow back as `command_response` messages correlated by `request_id`. Four commands: `browse_root_menu`, `browse_playlist`, `browse_all_tracks`, `load_track`. A Python `TrackScanner` orchestrator browses USB contents, loads tracks one at a time, captures Finder data via the existing read-only stream, and persists to a `bridge_scan_data` SQLite table. The bridge remains transport-only — no file I/O, no dedup logic, no scan state in Java. Python Layer 1 owns all orchestration and persistence.
Consequences: The bridge JAR must be rebuilt with the new `CommandHandler` class. The WebSocket `onMessage()` handler now routes to `CommandHandler` instead of being a no-op. The command channel adds `ConnectionManager` as a required Finder for menu browsing. Scan mode is an offline/prep workflow — not designed for use during live DJ sets. `bridge_scan_data` table is separate from `pioneer_metadata` (USB ANLZ data) to avoid conflating the two data sources.

## ADR-023: Analysis Status Chip Strip (replaces per-tier dot columns)
Date: 2026-04-03
Context: The library table previously displayed analysis tier completion as 5 individual single-character dot columns (Q, S, D, L, L-O) plus a separate Source badge. This communicated completion but not data source availability, tier prerequisites, or why a tier was unavailable. Users couldn't tell at a glance what analysis was possible for a given track.
Decision: Replace the 6 separate columns with a single "Analysis" column containing a compact chip strip. The strip has two zones: data source indicators (♪ Audio, ◆ Pioneer) and tier status chips (B, Q, S, D, L, LO). Each chip uses 3-state color coding: green (complete), amber (available — prerequisites met, not yet run), grey (unavailable — tooltip explains missing prerequisite). A TierSummaryBar above the table shows aggregate counts across all visible tracks. The expanded row (TrackPreviewRow) shows a full-label variant of the same component.
Consequences: The component (`AnalysisStatusChips`) is shared between table rows and detail views. Availability logic is derived from existing `TrackSummary` fields — no backend changes required. The chip strip doubles as a future launch point for triggering analysis (stretch goal: click amber chip to run).

## ADR-024: Analysis Status Header Badge (global progress indicator)
Date: 2026-04-03
Context: Batch analysis progress was only visible on the Library page via `StrataBatchProgress`. Navigating away from Library meant losing awareness of running analysis, risking accidental overlapping batches. There was no global indicator of system analysis load.
Decision: Add an `AnalysisStatusBadge` component mounted in the TopBar, driven by a dedicated `analysisProgressStore` Zustand store fed by `analysis_progress` WebSocket messages. The badge has five states: idle (hidden), single-track (spinner + track name + tier), batch (progress pill with count/bar/percentage + click-to-expand dropdown with current track detail and cancel button), complete (green pill, auto-fades after 10s), and failed/cancelled (amber/red pill, persists until dismissed). The store is independent (no cross-store imports) and the existing Library page `StrataBatchProgress` component is unchanged.
Consequences: The WS type union grows by one member (`WSAnalysisProgress`). Backend must broadcast `analysis_progress` messages from batch/single analysis runners (separate task). This lays groundwork for Phase 2 governance: queue awareness, priority rules, and concurrency limits.
