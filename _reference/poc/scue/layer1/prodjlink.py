...
Listens on Pioneer Pro DJ Link ports without requiring beat-link-trigger.
Auto-discovers the correct network interface, handling the macOS quirk where
binding to a unicast IP prevents broadcast UDP reception.
...
macOS broadcast quirk — see LEARNINGS.md
-----------------------------------------
Binding to a unicast IP (e.g. 169.254.20.47) on macOS silently prevents
reception of broadcast UDP packets from that interface. Fix: bind to ""
(INADDR_ANY) and use IP_BOUND_IF=25 to lock the socket to a specific interface.
"""
...
from .models import DeckState, STALE_TIMEOUT
...
def get_local_interfaces() -> list[dict]:
    ...
    Returns a list of dicts: { interface, ip, netmask, broadcast }
    """
    results = []
    ...
    if _HAS_NETIFACES:
        ...
        try:
            ...
            for res in _socket.getaddrinfo(hostname, None, _socket.AF_INET):
                ip = res[4][0]
...
class ProDJLinkClient:
    ...
    Binds UDP sockets using IP_BOUND_IF (macOS) or SO_BINDTODEVICE (Linux)
    to receive broadcast packets on the Pioneer interface.
    """
...
def _make_udp_socket(ip: str, port: int, iface_name: str = "") -> socket.socket:
    ...
    Linux uses SO_BINDTODEVICE for the same effect. Both fall back to unicast
    binding if the privileged option fails.
    ...
    if iface_name:
        if sys.platform == "darwin":
            # IP_BOUND_IF = 25 (net/in.h) — locks INADDR_ANY socket to one interface
            try:
                IP_BOUND_IF = 25
    ...
    # Fallback: bind to the unicast IP (works on Windows; limited on macOS/Linux for broadcasts)
    sock.bind((ip, port))
    return sock
