...
Fix: Bind to `""` (INADDR_ANY / 0.0.0.0) and use `setsockopt(IPPROTO_IP, IP_BOUND_IF=25, socket.if_nametoindex(iface_name))` to lock the socket to the specific interface. See `scue/layer1/prodjlink.py::_make_udp_socket()`.
...
Fix: Use librosa beat tracking as the working reference only. After Pioneer enrichment pass, replace the librosa beatgrid with Pioneer's hand-verified rekordbox grid. See `docs/ARCHITECTURE.md` — Sub-layer 1B enrichment pass.
...
Resolution: Replaced entirely with direct Pro DJ Link UDP parsing. No beat-link-trigger dependency.
