"""EDM arrangement flow model — Tier 1, Stage 2 of the analysis pipeline.

Maps generic music structure labels (verse, chorus, break, etc.) to EDM vocabulary
(drop, build, fakeout, breakdown) using energy profiles, spectral characteristics,
and section ordering priors.

TODO(milestone-1): upgrade from pure heuristics to a scoring model that evaluates
full section-label sequences against known EDM arrangement patterns:
  intro → verse → build → drop → breakdown → build → drop → outro
  intro → build → drop → breakdown → drop → outro
  etc.
See docs/ARCHITECTURE.md → Sub-layer 1A → "EDM flow model"
"""
...
def classify_sections(segments: list[dict], features: dict) -> list[dict]:
    ...
    Args:
        ...
        features: full feature dict from detectors/features.py::extract_all()
