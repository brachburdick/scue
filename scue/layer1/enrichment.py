...
TrackAnalysis:
  ...
  5. Store the updated analysis as a new versioned entry
...
def _compute_downbeats(beatgrid: list[float]) -> list[float]:
...
def run_enrichment_pass(
    ...
    Args:
        ...
        store: TrackStore for persisting the enriched version.
        cache: TrackCache for divergence logging and indexing.
        pioneer_beatgrid: beat timestamps from Pioneer (seconds), or None.
    ...
    enriched.source = "pioneer_enriched"
    enriched.beatgrid_source = "pioneer_enriched"
    enriched.enrichment_timestamp = time.time()

    # Store Pioneer values for reference
    enriched.pioneer_bpm = pioneer_bpm
    if pioneer_key:
        enriched.pioneer_key = pioneer_key
    if pioneer_beatgrid:
        enriched.pioneer_beatgrid = pioneer_beatgrid

    # ── BPM enrichment ───────────────────────────────────────────────
    bpm_ratio = 1.0
    if pioneer_bpm > 0 and abs(pioneer_bpm - analysis.bpm) > 0.05:
        log_divergence(
            cache, analysis.fingerprint, "bpm",
    ...
    if pioneer_beatgrid:
        if analysis.beats:
            log_divergence(
                cache, analysis.fingerprint, "beatgrid",
                f"{len(analysis.beats)} beats",
                f"{len(pioneer_beatgrid)} beats",
                resolution="pioneer_adopted",
            )
        enriched.beats = list(pioneer_beatgrid)
        enriched.downbeats = _compute_downbeats(pioneer_beatgrid)
    elif bpm_ratio != 1.0:
        # No Pioneer beatgrid — scale existing timestamps by BPM ratio
    ...
    if pioneer_key and pioneer_key != analysis.features.key:
        if analysis.features.key:
            log_divergence(
                cache, analysis.fingerprint, "key",
